...
export default function AboutPageView({ profile }: AboutPageViewProps) {
  ...
  const aboutText =
    profile.about_text || '故事还在继续，等你们慢慢把每一个温柔的日子写进这里。';
  const boyMessage =
    profile.boy_message_for_girl || '想对你说的话，会一直认真地放在心里。';
  const girlMessage =
    profile.girl_message_for_boy || '谢谢你出现在我的生活里，也谢谢你一直在。';
  ...
  const heroSubtitle =
    firstParagraph.length > 220 ? `${firstParagraph.slice(0, 160).trim()}…` : firstParagraph;

  const heroImage = profile.girl_avatar || profile.boy_avatar;
  ...
  const closingLine =
    paras.length > 0 ? paras[paras.length - 1] : '谢谢你走进我们的小世界。';

  return (
    <Box style={{ background: ARCHIVE.bg }}>
      <AboutHeroSection heroImageSrc={heroImage} subtitle={heroSubtitle} />
